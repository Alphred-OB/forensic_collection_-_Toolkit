<x-app-layout>
    ...
    <div class="py-6 space-y-6" x-data="{ showCreateModal: false, showEditModal: false, activeUser: {} }">
        ...
        <div class="bg-white p-4 rounded-lg shadow-sm border border-slate-200 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
            ...
            <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-col sm:flex-row flex-1 items-stretch sm:items-center gap-2 max-w-xl">
                <input type="text" name="search" value="{{ $search }}" placeholder="Search by name or email..." class="w-full sm:w-64 rounded-md border-slate-300 shadow-sm text-sm focus:border-blue-500 focus:ring-blue-500">
                <select name="role" class="w-full sm:w-40 rounded-md border-slate-300 shadow-sm text-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if($search || $role)
                        <a href="{{ route('admin.users.index') }}" class="flex-1 sm:flex-none px-3 py-2 bg-slate-200 text-slate-700 text-xs font-semibold rounded-md hover:bg-slate-300 transition text-center">Clear</a>
                    @endif
                </div>
            </form>

            <!-- Provision New User Button -->
            <button @click="showCreateModal = true" class="px-4 py-2 bg-blue-600 text-white rounded-md text-xs font-bold uppercase tracking-wider hover:bg-blue-700 shadow-sm transition text-center shrink-0">
